...
function TrainEvents() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10 px-4">
      <h1 className="text-3xl font-bold text-center text-green-700 mb-8">Available Train Tickets </h1>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
        {events.map(event => (
          <div key={event._id} className="bg-white p-6 rounded shadow">
            ...
            <Link
              to={`/events/${event._id}`}
              className="text-white bg-green-600 px-4 py-2 rounded hover:bg-green-700 inline-block mt-2"
            >
              View Details
            </Link>
          </div>
