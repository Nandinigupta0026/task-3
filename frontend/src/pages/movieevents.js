...
function MovieEvents() {
  ...
  useEffect(() => {
    const fetchMovieEvents = async () => {
      try {
        ...
        const movieEvents = res.data.filter(event => event.category === 'movie');
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10 px-4">
      <h1 className="text-3xl font-bold text-center text-blue-700 mb-8"> Now Showing: Movies</h1>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
        {events.map(event => (
          <div key={event._id} className="bg-white p-6 rounded shadow">
            <h2 className="text-xl font-semibold text-blue-600 mb-1">{event.title}</h2>
